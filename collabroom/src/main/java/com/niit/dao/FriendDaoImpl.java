...
public class FriendDaoImpl implements FriendDao {
	...
	public List<User> suggestedUsers(String email) {
		...
		String queryString="select * from user_20 where email in "
				+ "(select email from user_20 where email!=? "
				+ "minus "
				+ "(select toId_email from friend_20 where fromId_email=? "
				+ "union "
				+ "select fromId_email from friend_20 where toId_email=?))";
